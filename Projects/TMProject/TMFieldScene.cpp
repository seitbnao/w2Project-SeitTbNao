...
void TMFieldScene::UpdateScoreUI(unsigned int unFlag)
{
	...
	if (!(unFlag & 0x10))
	{
		...
		sprintf(szStr, "%d", pMobData->SkillPoint);
		if (m_pSkBonus)
...
int TMFieldScene::OnKeyShortSkill(char iCharCode, int lParam)
{
	if ((iCharCode < '0' || iCharCode > '9') && iCharCode != '!' && iCharCode != '@' && 
		iCharCode != '#' && iCharCode != '$' && iCharCode != '%' && iCharCode != 'ê' && 
...
int TMFieldScene::OnPacketAttack(PacketHeader* pStd)
{
	...
	if (pAttacker)
	{
		...
		{
			if (pAttack->SkillIndex == 4) // Possuêdo
			{
			...
			if (pAttack->SkillIndex == 98) // Canhêo Superior
			...
			else if (pAttack->SkillIndex == 45) // Arma Mêgica
			{
			...
			else if (pAttack->SkillIndex == 86) // Explosêo Etêrea
			{
				if (pAttacker)
				{
					TMVector3 vecPos{ pAttacker->m_vecPosition.x, pAttacker->m_fHeight + 0.5f, pAttacker->m_vecPosition.y };
