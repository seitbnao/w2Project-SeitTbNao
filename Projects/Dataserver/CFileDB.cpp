...
INT32 CFileDB::ProcessMessage(char *msg, INT32 conn)
{
	...
	case MSG_UPDATEWARDECLARATION://0x998
	{
        ...
        // Algum lêder declarou ou recusou guerra
		...
		{
			// O if ira entrar ao declarar pois normalmente a guerra nêo esta declarada
			...
			// O canal que ê o par, ou seja, quem recebe a declaração
			...
			for (int i = 0; i < MAX_SERVERGROUP; i++)
			{
				...
				// Envia o pacote para todos os canais disponêveis
	...
	case MSG_UPDATETOWERINFO://0x999
	{
		...
		// Para cada torre que cai em seu canal que nêo tenha sido ela a matar
		// A coroa paga 50 de fame
		// Desde que nêo seja o seu avanêo
		...
		if (g_pTowerWarState[conn].TowerState != conn)
		{
			...
			// Informa que quem deve atacar a torre ê o pessoal do canal
			g_pTowerWarState[conn].TowerState = conn;
			
			// Envia a mensagem para todos os canais se alguêm avanêou
			for (int i = 0; i < MAX_SERVERGROUP; i++)
			{
				if (pUser[i].Mode == USER_EMPTY)
					continue;
		...
		{
			// Matou a torre em seu prêprio canal
			// Caso queira, pode declarar a guerra e avanêar no outro canal
			// Desde que sua torre nêo morra antes disso

			// Informa que quem esta avanêando neste canal ê o outro
			g_pTowerWarState[conn].TowerState = otherConn;

			// Libera para possêvel avanêo no outro canal
			g_pTowerWarState[otherConn].TowerState = conn;

			// otherConn = 1
			// conn      = 0
			
			// Seta como ê possêvel declarar guerra novamente ao canal contrario
			...
			for (int i = 0; i < MAX_SERVERGROUP; i++)
			{
				...
				SendNotice(i, "O canal %d avanêa em seu territêrio.", conn + 1);
			}

			// Para avanêar no outro canal, basta declarar a guerra
		}

		// Enviar aos canais envolvidos suas novas informaçães
		// Enviar a notêcia de quem avanêou onde
		SendTowerWarInfo(2);
		break;
	}
	*/
