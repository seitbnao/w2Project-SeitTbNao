...
char* CUser::ReadMessage(int *ErrorCode, int* ErrorType)
{
    *ErrorCode = 0;
	...
	{
		if (User.Username[0])
			Log(clientId, LOG_INGAME, "Posi��o nRecvPosition - nProcPosition � invalida. Valor %d", (Socket.nRecvPosition - Socket.nProcPosition));
...
bool CUser::ReadMessage()
{
	...
	// Caso seja algo n�o valido, simplesmente retorna
	if (len <= 0)
		return false;
	...
	if (len == 4 || len == sizeof(p20D) + 4)
	{
		...
		{
			// Caso n�o esteja e tenha recebido o hello, ent�o
			// fecha a conex�o
			//CloseGame();
