...
bool CUser::RequestReqChallange(PacketHeader *Header)
{
	...
	{
		// Checa se o cabra ê lêder da cidade do NPC clicado
		if(ChargedGuildList[sServer.Channel - 1][level] == pMob[clientId].Mobs.Player.Guild && pMob[clientId].Mobs.Player.GuildLevel == 9)
		{
			Log(clientId, LOG_INGAME, "Clicou no NPC sendo lêder de Guild da cidade %d. Imposto: %I64d", level, g_pCityZone[level].impost);
			...
			if((level >= 0 && level <= 3 && sServer.WeekMode == 4) || (level == 4 && sServer.WarChannel == 1 && sServer.CastleState == 4))
			{
				...
				{ // gold maior que 1bi
					...
					// Quantidade de gold lêquido que sera dado ao usuario
					...
					if(static_cast<long long>(pMob[clientId].Mobs.Player.Coin) + gold > 2000000000LL)
					{
						...
						Log(clientId, LOG_INGAME, "Falha no recolhimento. Falta de espaêo para o gold lêquido! Gold atual: %d. Gold liquido: %I64d", pMob[clientId].Mobs.Player.Coin, gold);
						return true;
					}
					...
					if(bis > 59)
					{
						...
						if(null < (agrupped + alone))
						{
							...
							Log(clientId, LOG_INGAME, "Falha no recolhimento. Falta de espaêo para receber as baras. Agrupadas: %d. Sozinhas: %d. Espaêo: %d", agrupped, alone, null);
							return true;
						}
						...
						// Entrega as barras nêo agrupadas
						...
						Log(clientId, LOG_GUILD, "Imposto recolhido na cidade %d. Total gold: %I64d. Bis agrupados: %d. Bis soltos: %d.	Gold lêquido: %I64d", level, g_pCityZone[level].impost,
							agrupped, alone, gold);
						Log(SERVER_SIDE, LOG_GUILD, "Imposto recolhido na cidade %d por %s. Total gold: %I64d. Bis agrupados: %d. Bis soltos: %d.Gold lêquido: %I64d", level, pMob[clientId].Mobs.Player.MobName, g_pCityZone[level].impost,
							agrupped, alone, gold);
					}
					else
					{
						...
						if(null < bis)
						{
							...
							Log(clientId, LOG_INGAME, "Falha no recolhimento. Falta de espaêo para receber as baras. Barras: %d Espaêo: %d", bis, null);
							return true;
						}
						...
						Log(clientId, LOG_GUILD, "Imposto recolhido na cidade %d. Total gold: %I64d. Bis soltos: %d.Gold lêquido: %I64d", level, g_pCityZone[level].impost,
							bis, gold);
						Log(SERVER_SIDE, LOG_GUILD, "Imposto recolhido na cidade %d por %s. Total gold: %I64d. Bis soltos: %d.Gold lêquido: %I64d", level, pMob[clientId].Mobs.Player.MobName, g_pCityZone[level].impost,
							bis, gold);
					}
				...
				else
				{
					...
					Log(clientId, LOG_GUILD, "Imposto recolhido na cidade %d. Total gold: %I64d. Gold lêquido: %I64d", level, g_pCityZone[level].impost, gold);
					Log(SERVER_SIDE, LOG_GUILD, "Imposto recolhido na cidade %d por %s. Total gold: %I64d. Gold lêquido: %I64d", level, pMob[clientId].Mobs.Player.MobName, g_pCityZone[level].impost, gold);
				}
				...
				// Se chegou atê esta parte do cêdigo quer dizer que 
				// era possêvel recolher os impostos de Noatun
				if(level == 4)
