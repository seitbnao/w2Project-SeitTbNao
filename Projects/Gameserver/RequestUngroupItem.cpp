...
bool CUser::RequestUngroupItem(PacketHeader *Header)
{
	...
	if(Slot == -1) 
	{
		SendClientMessage(clientId, "Nêo ha espaêo para separar!");
	...
	if (!canUngroup)
	{
		SendClientMessage(p->Header.ClientId, "Nêo foi possêvel desagrupar");
	...
	for(int i = 0;i < 3;i++)
	{
		...
		{
			SendClientMessage(clientId, "Item imêvel!");
	...
	for(int i = 0;i < 3;i++)
	{
		if(pMob[clientId].Mobs.Player.Inventory[p->SlotID].stEffect[i].cEffect == EF_AMOUNT)
		{
			...
			{
				SendClientMessage(p->Header.ClientId, "Nêo pode separar mais do que se tem.");
