...
bool CUser::RequestEssenceUse(PacketHeader* header)
{
	...
	if (amount < p->Amount || p->Amount < 0 || p->Amount > 120)
	{
		...
		SendItem(clientId, SlotType::Inv, p->Slot, srcItem);
		Log(SERVER_SIDE, LOG_INGAME, "Valor para uso de êmagos incorreto. Total de amount: %d. Valor enviado: %d", amount, p->Amount);
		return true;
	}

	if (player->Equip[14].EFV2 <= 5)
	{
		SendClientMessage(clientId, "Vitalidade mênima para aumentar o nêvel da montaria ê 6.");

		SendItem(clientId, SlotType::Inv, p->Slot, srcItem);
		return true;
	}

	Log(clientId, LOG_INGAME, "Solicitado o uso de %d êmagos", p->Amount);
	...
	for (int i = 0; i < p->Amount; ++i)
	{
		if (mount < 2360)
		{
			...
			{
				Log(clientId, LOG_INGAME, "êmago obteve sucesso de %s %s.",
		...
		else
		{
			...
			if (_rand > _rate)
			{
				...
				{
					Log(clientId, LOG_INGAME, "Falhou %s e nada aconteceu. %s. Level atual: %hhu. Bênus: %d", 
						g_pItemList[srcItem->sIndex].ItemName,
						player->Equip[14].toString().c_str(), 
						player->Equip[14].EF2,
						_bonus);
				...
				{
					Log(clientId, LOG_INGAME, "%s falhou e level baixou %s. Level atual: %hhu, Level novo: %hhu Bênus: %d", 
						g_pItemList[srcItem->sIndex].ItemName,
						player->Equip[14].toString().c_str(), 
						player->Equip[14].EF2, 
						player->Equip[14].EF2 - 1, 
			...
			{
				player->Equip[14].EF2++;
				Log(clientId, LOG_INGAME, "%s obteve sucesso de %s. Level atual: %hhu, Level novo: %hhu. Bênus %d", 
					g_pItemList[srcItem->sIndex].ItemName,
					player->Equip[14].toString().c_str(), 
					player->Equip[14].EF2 - 1,
					player->Equip[14].EF2);
	...
	SendClientMessage(clientId, "êmagos usados: %d. Com sucesso: %d. Com falha: %d Leveis ganhos: %d", success + fail, success, fail, player->Equip[14].EF2 - lastLevel);
